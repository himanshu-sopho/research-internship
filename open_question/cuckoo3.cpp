...
int hash=111;	//initially hash function uses 11 for modulo
int n,k;
int g_counter=0;
int cntr=0;
int p=0;
...
int insert_1(vector<vector<int> > &a,vector<int> &c,int number)		//first hash function
{
	if (count>(n*hash+1))						//check for infinite loop
	{
		...
		if (cntr==0)
			...
			cntr++;
			g_counter=counter;	
			p=(g_counter*100)/(n*hash);
		//	cout << "threshold " << (g_counter*100)/(n*hash); 
			}
	...
	int key = ((k+1)*(k+1)*number*number+(k+1)*number+130)%hash;						//key to decide the bucket 
